...
export default function ProductManager({ initialProducts, categories }: Props) {
  ...
  const handleUploadFiles = async (files: File[], isGallery: boolean) => {
    ...
    for (const file of files) {
      if (file.size > 5 * 1024 * 1024) { showToast('err', `${file.name} exceeds 5MB limit.`); return; }
      if (!['image/jpeg', 'image/png', 'image/webp', 'image/jpg'].includes(file.type)) { showToast('err', `${file.name} format not supported (use JPG/PNG/WEBP).`); return; }
    }
    setLoading(true);
    try {
      ...
      if (isGallery) {
        ...
        setCurrent(p => ({ ...p, images: JSON.stringify([...arr, ...uploadedUrls]) }));
      } else {
    ...
    } finally { setImagePreview(null); setLoading(false); }
  };

  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, isGallery: boolean) => {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;
    if (!isGallery && files[0]) setImagePreview(URL.createObjectURL(files[0]));
  ...
  const handleDrop = async (event: React.DragEvent<HTMLDivElement>, isGallery: boolean) => {
    ...
    await handleUploadFiles(Array.from(event.dataTransfer.files || []), isGallery);
  };

  const removeGalleryImage = (url: string) => {
    let arr: string[] = [];
    try { arr = JSON.parse(current.images || '[]'); } catch {}
    setCurrent(p => ({ ...p, images: JSON.stringify(arr.filter(i => i !== url)) }));
  };
  ...
  const handleSave = async (asDraft = false, saveAndAdd = false) => {
    ...
    if (!resolvedCode && resolvedCategoryId > 0) {
      ...
      try {
        ...
        if ((res as any).success && (res as any).code) resolvedCode = (res as any).code;
      } catch {}
    }
    if (!resolvedCode) resolvedCode = buildFallbackCode(fallbackCategory?.slug);
    if (!current.categoryId && resolvedCategoryId > 0) setCurrent(p => ({ ...p, categoryId: resolvedCategoryId }));
    ...
    try {
      ...
      try { const parsed = JSON.parse(current.images || '[]'); if (Array.isArray(parsed)) resolvedImages = parsed.filter(Boolean); } catch {}
      if (current.featuredImage) { resolvedImages = resolvedImages.filter(i => i !== current.featuredImage); resolvedImages.unshift(current.featuredImage); }

      const payload = {
        ...current, name: trimmedName, code: resolvedCode,
      ...
      if ((res as any).success) {
        ...
        if (saveAndAdd) { setCurrent({ ...emptyProduct(), categoryId: categories[0]?.id ?? 0 }); setActiveSection('basic'); }
  ...
  const handleDelete = async (id: number) => {
    ...
    setLoading(false);
    if ((res as any).success) { setProducts(p => p.filter(x => x.id !== id)); showToast('ok', 'Product deleted.'); }
    else showToast('err', (res as any).error || 'Delete failed.');
  };
  ...
  const handleDuplicate = async (id: number) => {
    ...
    setLoading(false);
    if ((res as any).success) { showToast('ok', 'Product duplicated.'); setTimeout(() => window.location.reload(), 600); }
    else showToast('err', (res as any).error || 'Duplicate failed.');
  };
  ...
  const handleBulkDelete = async () => {
    ...
    setLoading(false);
    if ((res as any).success) { setProducts(p => p.filter(x => !selected.has(x.id))); setSelected(new Set()); showToast('ok', `Deleted ${(res as any).count} product(s).`); }
    else showToast('err', (res as any).error || 'Bulk delete failed.');
  };
  ...
  const handleBulkUpdate = async (updates: any) => {
    ...
    setLoading(false);
    if ((res as any).success) { showToast('ok', `Updated ${(res as any).count} product(s).`); setSelected(new Set()); setTimeout(() => window.location.reload(), 600); }
  };

  const toggleSelect = (id: number) => { const next = new Set(selected); if (next.has(id)) next.delete(id); else next.add(id); setSelected(next); };
  const toggleSelectAll = () => { if (selected.size === paginated.length) setSelected(new Set()); else setSelected(new Set(paginated.map(p => p.id))); };
  ...
  const handleImport = async () => {
    ...
    setLoading(false);
    if ((res as any).success) { showToast('ok', `Imported ${(res as any).count} products.`); setImportContent(''); setShowImport(false); setTimeout(() => window.location.reload(), 800); }
    else showToast('err', (res as any).error || 'Import failed.');
  };
  ...
  const parseImg = (s: string): string => {
    ...
    try {
      ...
      if (Array.isArray(a) && a.length) { const firstValid = a.find((item: unknown) => typeof item === 'string' && item.trim().length > 0); return firstValid || 'https://images.unsplash.com/photo-1510519138101-570d1dca3d66?auto=format&fit=crop&q=80&w=200'; }
    } catch {}
    if (typeof s === 'string' && s.trim().startsWith('[')) return 'https://images.unsplash.com/photo-1510519138101-570d1dca3d66?auto=format&fit=crop&q=80&w=200';
    return s;
  };
    ...
    <div className="space-y-5">
      ...
      {toast && (
        <div className={`fixed top-6 right-6 z-[200] flex items-center gap-2 px-5 py-3.5 rounded-xl shadow-2xl text-xs font-bold border-2 ${
          ...
          {toast.type === 'ok' ? <CheckCircle2 className="w-5 h-5 text-green-600" /> : toast.type === 'err' ? <AlertCircle className="w-5 h-5 text-red-600" /> : <AlertCircle className="w-5 h-5 text-blue-600" />}
          {toast.msg}
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        <div className="flex flex-wrap justify-between items-start gap-4">
          <div>
            <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2"><Package className="w-6 h-6 text-secondary" /> Product Management</h1>
            <p className="text-xs text-gray-500 mt-1.5 max-w-xl">Manage all wholesale products, stock levels, pricing, suppliers, and product images from one place.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={handleOpenAdd} className="bg-primary hover:bg-blue-800 text-white font-bold px-4 py-2.5 rounded-lg text-xs flex items-center gap-2 shadow-md transition-all hover:scale-105"><Plus className="w-4 h-4" /> Add Product</button>
            <button onClick={() => setShowImport(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-4 py-2.5 rounded-lg text-xs flex items-center gap-2 shadow-md"><Upload className="w-4 h-4" /> Import</button>
            <button onClick={() => handleExportCSV(filtered)} className="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold px-4 py-2.5 rounded-lg text-xs flex items-center gap-2"><Download className="w-4 h-4" /> Export</button>
            <button onClick={() => window.print()} className="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold px-4 py-2.5 rounded-lg text-xs flex items-center gap-2"><Printer className="w-4 h-4" /> Print</button>
            <button onClick={() => window.location.reload()} className="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold px-4 py-2.5 rounded-lg text-xs flex items-center gap-2"><RefreshCw className="w-4 h-4" /> Refresh</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-9 gap-3">
        {[
          ...
          <div key={i} className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm hover:shadow-md transition-all">
            ...
            <p className={`${(card as any).big ? 'text-sm' : 'text-xl'} font-black text-gray-900 leading-tight`}>{card.v}</p>
          </div>
        ))}
      </div>
      ...
      {alerts.length > 0 && showAlerts && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4">
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-xs font-black text-yellow-800 uppercase flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> Stock Alerts ({alerts.length})</h3>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search by name, code, barcode, brand, supplier..."
          ...
          <select value={catFilter} onChange={e => { setCatFilter(e.target.value); setPage(1); }} className="text-xs border-2 border-gray-200 rounded-lg px-3 py-2.5 outline-none focus:border-primary bg-white font-semibold cursor-pointer">
            <option value="all">All Categories</option>
            {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <select value={stockFilter} onChange={e => { setStockFilter(e.target.value); setPage(1); }} className="text-xs border-2 border-gray-200 rounded-lg px-3 py-2.5 outline-none focus:border-primary bg-white font-semibold cursor-pointer">
            <option value="all">All Stock</option>
            <option value="In Stock">In Stock</option>
            <option value="Low Stock">Low Stock</option>
            <option value="Out of Stock">Out of Stock</option>
          </select>
          <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }} className="text-xs border-2 border-gray-200 rounded-lg px-3 py-2.5 outline-none focus:border-primary bg-white font-semibold cursor-pointer">
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="text-xs border-2 border-gray-200 rounded-lg px-3 py-2.5 outline-none focus:border-primary bg-white font-semibold cursor-pointer">
        ...
        {selected.size > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-2 items-center">
            ...
            <button onClick={handleBulkDelete} className="bg-red-100 hover:bg-red-200 text-red-700 px-3 py-1.5 text-[11px] font-bold rounded-md flex items-center gap-1"><Trash2 className="w-3.5 h-3.5" /> Delete</button>
            <button onClick={() => handleBulkUpdate({ isActive: true })} className="bg-green-100 hover:bg-green-200 text-green-700 px-3 py-1.5 text-[11px] font-bold rounded-md flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> Activate</button>
            <button onClick={() => handleBulkUpdate({ isActive: false })} className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 text-[11px] font-bold rounded-md flex items-center gap-1"><X className="w-3.5 h-3.5" /> Deactivate</button>
            <button onClick={() => handleBulkUpdate({ isFeatured: true })} className="bg-yellow-100 hover:bg-yellow-200 text-yellow-800 px-3 py-1.5 text-[11px] font-bold rounded-md flex items-center gap-1"><Star className="w-3.5 h-3.5" /> Mark Featured</button>
            <button onClick={() => handleExportCSV(filtered.filter(p => selected.has(p.id)))} className="bg-blue-100 hover:bg-blue-200 text-blue-700 px-3 py-1.5 text-[11px] font-bold rounded-md flex items-center gap-1"><Download className="w-3.5 h-3.5" /> Export Selected</button>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full text-xs">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 font-bold uppercase text-[10px] tracking-wider">
              <tr>
                <th className="px-3 py-3.5 text-left w-10"><input type="checkbox" checked={selected.size > 0 && selected.size === paginated.length} onChange={toggleSelectAll} className="cursor-pointer" /></th>
              ...
              {paginated.map(p => {
                ...
                return (
                  <tr key={p.id} className={`hover:bg-gray-50/80 transition-colors ${isSelected ? 'bg-blue-50/60' : ''} ${!p.isActive ? 'opacity-50' : ''}`}>
                    <td className="px-3 py-3"><input type="checkbox" checked={isSelected} onChange={() => toggleSelect(p.id)} className="cursor-pointer" /></td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <span className="font-black text-gray-800 block">{p.stockQuantity} pcs</span>
                      <span className="text-[9px] text-blue-600 font-bold">{stockCartons} cartons</span>
                    </td>
                    <td className="px-3 py-3 text-center">
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap ${p.stockStatus === 'In Stock' ? 'bg-green-100 text-green-700' : p.stockStatus === 'Low Stock' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>{p.stockStatus}</span>
                    ...
                    <td className="px-3 py-3 text-center">
                      <div className="flex justify-center gap-1">
                        <button onClick={() => handleOpenEdit(p)} title="Edit" className="text-blue-600 hover:bg-blue-100 p-1.5 rounded-md"><Edit className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDuplicate(p.id)} title="Duplicate" className="text-gray-500 hover:bg-gray-200 p-1.5 rounded-md"><Copy className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDelete(p.id)} title="Delete" className="text-red-500 hover:bg-red-100 p-1.5 rounded-md"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                );
        ...
        {totalPages > 1 && (
          <div className="border-t border-gray-100 p-4 flex justify-between items-center text-xs">
            <span className="text-gray-600 font-semibold">Showing {((page - 1) * perPage) + 1}-{Math.min(page * perPage, filtered.length)} of {filtered.length}</span>
            <div className="flex gap-1">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50 disabled:opacity-40 font-bold">Prev</button>
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                const pNum = i + 1;
                return <button key={pNum} onClick={() => setPage(pNum)} className={`px-3 py-1.5 rounded-md font-bold ${page === pNum ? 'bg-primary text-white' : 'border border-gray-200 hover:bg-gray-50'}`}>{pNum}</button>;
              })}
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50 disabled:opacity-40 font-bold">Next</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl my-auto flex flex-col max-h-[95vh]">
            ...
            <div className="bg-gradient-to-r from-[#0B2C63] to-[#1a4fa8] text-white px-6 py-4 flex justify-between items-center rounded-t-2xl flex-shrink-0">
              <div>
                <h3 className="font-black text-base uppercase tracking-tight">{current.id ? 'Edit Product' : 'Add New Product'}</h3>
                <p className="text-[10px] text-blue-200 font-medium">Fill in product details across all sections</p>
              </div>
              <button onClick={() => setShowModal(false)} className="hover:text-secondary p-2 hover:bg-white/10 rounded-lg"><X className="w-5 h-5" /></button>
            </div>

            {/* Section tabs */}
            <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex-shrink-0 overflow-x-auto">
              <div className="flex gap-1 min-w-max">
                {(['basic', 'images', 'pricing', 'stock', 'attributes', 'delivery', 'visibility'] as const).map(s => (
                  <button key={s} onClick={() => setActiveSection(s)}
                    className={`px-3 py-1.5 rounded-full text-[11px] font-bold capitalize transition-all ${activeSection === s ? 'bg-primary text-white shadow' : 'text-gray-600 hover:bg-gray-200'}`}>
                    {s === 'basic' ? '📋 Basic' : s === 'images' ? '🖼️ Images' : s === 'pricing' ? '💰 Pricing' : s === 'stock' ? '📦 Stock' : s === 'attributes' ? '🏷️ Attributes' : s === 'delivery' ? '🚚 Delivery' : '👁️ Visibility'}
                  </button>
                ))}
              </div>
            </div>
              ...
              <div className="p-6 space-y-5 text-xs">
                  ...
                  <div className="space-y-5 bg-white p-5 rounded-xl border border-gray-200">
                    ...
                    <div className="bg-blue-50/50 border-2 border-blue-100 rounded-xl p-4 flex flex-col sm:flex-row items-start gap-6">
                      ...
                      <div className="flex-1 text-left">
                        <h4 className="font-black text-gray-800 text-sm flex items-center gap-2"><Star className="w-4 h-4 text-secondary" /> Product Image</h4>
                        <p className="text-[11px] text-gray-500 mt-1.5 leading-relaxed">Upload a clear product image. This appears on the homepage, product listings, and admin list.</p>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Buying Price (KSh) *</label>
                        ...
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Selling Price / Piece (KSh) *</label>
                      ...
                      {/* ── PIECES PER CARTON ── */}
                      <div>
                        <label className="block font-bold text-gray-700 mb-1">Pieces per Carton *</label>
                        <input
                          type="number" min="1"
                          value={current.qtyPerCarton || 1}
                      ...
                      <div>
                        <label className="block font-bold text-gray-700 mb-1">Current Stock (total pieces)</label>
                        <input
                          type="number" min="0"
                          value={current.stockQuantity ?? 100}
                          onChange={e => setCurrent(p => ({ ...p, stockQuantity: Number(e.target.value) }))}
                          className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white"
                        />
                        {/* Stock → cartons breakdown */}
                        {(Number(current.stockQuantity) > 0) && (Number(current.qtyPerCarton) > 0) && (
                          <p className="text-[10px] text-gray-500 mt-1">
                            = {Math.floor(Number(current.stockQuantity) / Number(current.qtyPerCarton))} carton{Math.floor(Number(current.stockQuantity) / Number(current.qtyPerCarton)) !== 1 ? 's' : ''}
                            {(Number(current.stockQuantity) % Number(current.qtyPerCarton)) > 0
                              ? ` + ${Number(current.stockQuantity) % Number(current.qtyPerCarton)} loose piece${(Number(current.stockQuantity) % Number(current.qtyPerCarton)) !== 1 ? 's' : ''}`
                              : ''}
                          </p>
                        )}
                      </div>
                ...
                {activeSection === 'images' && (
                  <div className="space-y-5">
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      <h4 className="font-black text-secondary uppercase tracking-wider text-[11px] mb-3 flex items-center gap-2"><ImageIcon className="w-3.5 h-3.5" /> Gallery Images ({galleryImages.length})</h4>
                      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                        ...
                        <label className="aspect-square border-2 border-dashed border-gray-300 hover:border-primary rounded-lg flex flex-col items-center justify-center cursor-pointer bg-gray-50 hover:bg-blue-50/50 transition-colors">
                          <input type="file" className="hidden" accept="image/*" multiple onChange={e => handleUpload(e, true)} />
                          <Plus className="w-6 h-6 text-gray-400" /><p className="text-[10px] text-gray-500 font-bold mt-1">Add Images</p>
                        </label>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          ...
                          <input type="number" step="0.01" value={current.buyingPriceRmb || ''} onChange={e => setCurrent(p => ({ ...p, buyingPriceRmb: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" />
                        </div>
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Exchange Rate (KES per RMB)</label>
                          <input type="number" step="0.01" value={current.exchangeRate || ''} onChange={e => setCurrent(p => ({ ...p, exchangeRate: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" />
                        </div>
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Buying Price (KES) *</label>
                          <input ref={(el) => { fieldRefs.current.buyingPrice = el; }} type="number" required value={current.buyingPrice || ''} onChange={e => setCurrent(p => ({ ...p, buyingPrice: Number(e.target.value) }))} className={`w-full p-2.5 border-2 rounded-lg outline-none focus:border-primary bg-blue-50 font-bold text-primary ${validationErrors.buyingPrice ? 'border-red-300' : 'border-gray-200'}`} />
                        </div>
                      </div>
                      <div className="grid grid-cols-3 gap-3 mt-4">
                        <div><label className="block font-bold text-gray-700 mb-1">Transport Cost</label><input type="number" value={current.transportCost || ''} onChange={e => setCurrent(p => ({ ...p, transportCost: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Import Cost</label><input type="number" value={current.importCost || ''} onChange={e => setCurrent(p => ({ ...p, importCost: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Other Expenses</label><input type="number" value={current.otherExpenses || ''} onChange={e => setCurrent(p => ({ ...p, otherExpenses: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                          ...
                          <input ref={(el) => { fieldRefs.current.wholesalePrice = el; }} type="number" required value={current.wholesalePrice || ''} onChange={e => setCurrent(p => ({ ...p, wholesalePrice: Number(e.target.value) }))} className={`w-full p-2.5 border-2 rounded-lg outline-none focus:border-primary bg-white font-black text-primary ${validationErrors.wholesalePrice ? 'border-red-300' : 'border-primary'}`} />
                        </div>
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Retail Price</label>
                          <input type="number" value={current.retailPrice || ''} onChange={e => setCurrent(p => ({ ...p, retailPrice: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" />
                        </div>
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Discount Price</label>
                          <input type="number" value={current.discountPrice || ''} onChange={e => setCurrent(p => ({ ...p, discountPrice: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-red-300 bg-white text-red-600 font-bold" />
                        </div>
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">VAT %</label>
                          <input type="number" value={current.vatPercent || 16} onChange={e => setCurrent(p => ({ ...p, vatPercent: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-5">
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                        <div><label className="block font-bold text-gray-700 mb-1">Current Stock (pcs) *</label><input type="number" required value={current.stockQuantity || 0} onChange={e => setCurrent(p => ({ ...p, stockQuantity: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white font-bold text-primary" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Opening Stock</label><input type="number" value={current.openingStock || 0} onChange={e => setCurrent(p => ({ ...p, openingStock: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Min Stock</label><input type="number" value={current.minStockLevel || 5} onChange={e => setCurrent(p => ({ ...p, minStockLevel: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Max Stock</label><input type="number" value={current.maxStockLevel || 500} onChange={e => setCurrent(p => ({ ...p, maxStockLevel: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Reorder Level</label><input type="number" value={current.reorderLevel || 10} onChange={e => setCurrent(p => ({ ...p, reorderLevel: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      </div>

                      {/* ── Stock → Cartons live breakdown ── */}
                      {(Number(current.stockQuantity) > 0) && (Number(current.qtyPerCarton) > 0) && (
                        <div className="mt-3 flex items-center gap-3 px-4 py-2.5 bg-blue-50 border border-blue-100 rounded-lg text-xs text-blue-700 font-medium">
                          <span>📦</span>
                          <span>
                            {Number(current.stockQuantity)} pieces
                            = <strong>{Math.floor(Number(current.stockQuantity) / Number(current.qtyPerCarton))} carton{Math.floor(Number(current.stockQuantity) / Number(current.qtyPerCarton)) !== 1 ? 's' : ''}</strong>
                            {(Number(current.stockQuantity) % Number(current.qtyPerCarton)) > 0
                              ? ` + ${Number(current.stockQuantity) % Number(current.qtyPerCarton)} loose piece${(Number(current.stockQuantity) % Number(current.qtyPerCarton)) !== 1 ? 's' : ''}`
                              : ''}
                          </span>
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                          <label className="block font-bold text-gray-700 mb-1">Pieces per Carton *</label>
                          <input ref={(el) => { fieldRefs.current.qtyPerCarton = el; }} type="number" value={current.qtyPerCarton || 1} onChange={e => setCurrent(p => ({ ...p, qtyPerCarton: Number(e.target.value) }))} className={`w-full p-2.5 border-2 rounded-lg outline-none focus:border-primary bg-white font-bold text-blue-700 ${validationErrors.qtyPerCarton ? 'border-red-300' : 'border-blue-300'}`} />
                          {Number(current.wholesalePrice) > 0 && (
                            <p className="text-[10px] text-blue-600 font-bold mt-1">
                              Carton price = {fmt((Number(current.wholesalePrice) || 0) * (Number(current.qtyPerCarton) || 1))}
                            </p>
                          )}
                        </div>
                        <div><label className="block font-bold text-gray-700 mb-1">Middle Pack</label><input type="number" value={current.middlePack || ''} onChange={e => setCurrent(p => ({ ...p, middlePack: e.target.value ? Number(e.target.value) : undefined as any }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Pieces / Pack</label><input type="number" value={current.piecesPerPack || ''} onChange={e => setCurrent(p => ({ ...p, piecesPerPack: e.target.value ? Number(e.target.value) : undefined as any }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Unit</label>
                          <select value={current.unit || 'pcs'} onChange={e => setCurrent(p => ({ ...p, unit: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white">
                            <option value="pcs">Pieces</option><option value="kg">Kilograms</option><option value="ltr">Liters</option><option value="m">Meters</option><option value="boxes">Boxes</option><option value="dozens">Dozens</option>
                          </select>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div><label className="block font-bold text-gray-700 mb-1">Weight (kg)</label><input ref={(el) => { fieldRefs.current.weight = el; }} type="number" step="0.01" value={current.weight || ''} onChange={e => setCurrent(p => ({ ...p, weight: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Length (cm)</label><input type="number" step="0.1" value={current.length || ''} onChange={e => setCurrent(p => ({ ...p, length: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Width (cm)</label><input type="number" step="0.1" value={current.width || ''} onChange={e => setCurrent(p => ({ ...p, width: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Height (cm)</label><input type="number" step="0.1" value={current.height || ''} onChange={e => setCurrent(p => ({ ...p, height: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div><label className="block font-bold text-gray-700 mb-1">Warehouse</label><input value={current.warehouse || 'Main Warehouse'} onChange={e => setCurrent(p => ({ ...p, warehouse: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Storage Location</label><input value={current.storageLocation || ''} onChange={e => setCurrent(p => ({ ...p, storageLocation: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                        <div><label className="block font-bold text-gray-700 mb-1">Shelf Number</label><input value={current.shelfNumber || ''} onChange={e => setCurrent(p => ({ ...p, shelfNumber: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeSection === 'attributes' && (
                  <div className="bg-white p-5 rounded-xl border border-gray-200">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div><label className="block font-bold text-gray-700 mb-1">Color</label><input value={current.color || ''} onChange={e => setCurrent(p => ({ ...p, color: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Material</label><input value={current.material || ''} onChange={e => setCurrent(p => ({ ...p, material: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Size</label><input value={current.size || ''} onChange={e => setCurrent(p => ({ ...p, size: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Packaging Type</label><input value={current.packagingType || ''} onChange={e => setCurrent(p => ({ ...p, packagingType: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4">
                      <div><label className="block font-bold text-gray-700 mb-1">Serial Number</label><input value={current.serialNumber || ''} onChange={e => setCurrent(p => ({ ...p, serialNumber: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Batch Number</label><input value={current.batchNumber || ''} onChange={e => setCurrent(p => ({ ...p, batchNumber: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Manufacturing Date</label><input type="date" value={current.manufacturingDate ? String(current.manufacturingDate).slice(0,10) : ''} onChange={e => setCurrent(p => ({ ...p, manufacturingDate: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Expiration Date</label><input type="date" value={current.expirationDate ? String(current.expirationDate).slice(0,10) : ''} onChange={e => setCurrent(p => ({ ...p, expirationDate: e.target.value }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                      <div><label className="block font-bold text-gray-700 mb-1">Tags (comma separated)</label><input value={current.tags || ''} onChange={e => setCurrent(p => ({ ...p, tags: e.target.value }))} placeholder="hardware, tools, professional" className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">SEO Keywords</label><input value={current.keywords || ''} onChange={e => setCurrent(p => ({ ...p, keywords: e.target.value }))} placeholder="best lock, padlock kenya, security" className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                    </div>
                  </div>
                )}
                ...
                {activeSection === 'delivery' && (
                  <div className="bg-white p-5 rounded-xl border border-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div><label className="block font-bold text-gray-700 mb-1">Delivery Time</label><input value={current.deliveryTime || ''} onChange={e => setCurrent(p => ({ ...p, deliveryTime: e.target.value }))} placeholder="e.g. 1-3 days" className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Shipping Weight (kg)</label><input type="number" step="0.01" value={current.shippingWeight || ''} onChange={e => setCurrent(p => ({ ...p, shippingWeight: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                      <div><label className="block font-bold text-gray-700 mb-1">Shipping Charges (KES)</label><input type="number" value={current.shippingCharges || ''} onChange={e => setCurrent(p => ({ ...p, shippingCharges: Number(e.target.value) }))} className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                    </div>
                    <div className="mt-4"><label className="block font-bold text-gray-700 mb-1">Courier Options (comma separated)</label><input value={current.courierOptions || ''} onChange={e => setCurrent(p => ({ ...p, courierOptions: e.target.value }))} placeholder="G4S, Wells Fargo, Easy Coach" className="w-full p-2.5 border-2 border-gray-200 rounded-lg outline-none focus:border-primary bg-white" /></div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                      {[{ k: 'freeDelivery', l: '🚚 Free Delivery' }, { k: 'fragile', l: '⚠️ Fragile Item' }, { k: 'returnable', l: '🔄 Returnable' }, { k: 'cashOnDelivery', l: '💵 Cash On Delivery' }].map(b => (
                        <label key={b.k} className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border-2 cursor-pointer transition-all ${(current as any)[b.k] ? 'bg-primary/10 border-primary' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
            ...
            <div className="bg-white px-6 py-4 border-t border-gray-200 flex flex-wrap justify-end gap-2 flex-shrink-0">
              <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2.5 rounded-lg font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 text-xs">Cancel</button>
              <button type="button" onClick={() => setCurrent({ ...emptyProduct(), categoryId: categories[0]?.id ?? 0 })} className="px-4 py-2.5 rounded-lg font-bold text-gray-600 bg-white border border-gray-300 hover:bg-gray-50 text-xs flex items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5" /> Reset</button>
              <button type="button" onClick={() => handleSave(true)} className="px-4 py-2.5 rounded-lg font-bold text-gray-700 bg-gray-200 hover:bg-gray-300 text-xs flex items-center gap-1.5"><Archive className="w-3.5 h-3.5" /> Save as Draft</button>
              <button type="button" onClick={() => handleSave(false, true)} className="px-4 py-2.5 rounded-lg font-bold text-primary bg-blue-100 hover:bg-blue-200 text-xs flex items-center gap-1.5"><Plus className="w-3.5 h-3.5" /> Save & Add Another</button>
              <button type="button" onClick={() => handleSave(false)} className="bg-gradient-to-r from-secondary to-orange-500 hover:from-orange-500 hover:to-secondary text-white font-bold px-6 py-2.5 rounded-lg text-xs flex items-center gap-1.5 shadow-md"><Save className="w-3.5 h-3.5" /> {current.id ? 'Update Product' : 'Save Product'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs">
                <p className="font-bold text-primary mb-1">CSV Format:</p>
                <code className="block text-[10px] bg-white p-2 rounded font-mono">code,barcode,name,category,brand,supplier,description,buying_price,price,carton_qty,stock_quantity,image</code>
              </div>
              <textarea rows={10} value={importContent} onChange={e => setImportContent(e.target.value)} placeholder="Paste CSV or JSON data here..." className="w-full p-3 font-mono text-[11px] border-2 border-gray-200 rounded-lg outline-none focus:border-primary resize-none" />
              <div className="flex justify-end gap-2">
                ...
                <button onClick={handleImport} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6 py-2.5 rounded-lg text-xs flex items-center gap-2"><Upload className="w-4 h-4" /> Process Import</button>
              </div>
            </div>
          </div>
        </div>
